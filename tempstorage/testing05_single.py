...
PROMPT_PATH = "/mnt/common/lx/Temp01/retry_with_answer_prompt.txt"
OUTPUT_PATH = "/mnt/common/lx/Temp01/testing05_single_output.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="单题测试：题目 + 失败解 + 标准答案 -> LLM 重试")
    ...
    parser.add_argument("--prompt-path", type=str, default=PROMPT_PATH, help="Prompt 模板路径")
    parser.add_argument("--dataset-path", type=str, default=DATASET_PATH, help="HF 数据集路径")
    parser.add_argument("--output-path", type=str, default=OUTPUT_PATH, help="输出 JSON 路径")
    ...
    print(f"开始单题测试: dataset_idx={resolved_idx}, unique_id={case.get('unique_id')}")
    try:
        retry_output = call_chat_completion(BASE_URL, API_KEY, payload)
        out = {
